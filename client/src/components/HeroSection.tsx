...
const HeroSection = () => {
  ...
  const [searchType, setSearchType] = useState('الجامعة');
  const [searchQuery, setSearchQuery] = useState(queryParam);

  // تحديث حقل البحث تلقائيًا عند تغيير الرابط
  useEffect(() => {
  ...
  const handleSearch = () => {
    ...
    switch (searchType) {
      ...
      case 'الدورة':
        ...
        break;
      case 'مركز اللغة':
        pagePath = '/language-centers';
        break;
      default:
        pagePath = '/';
    }

    router.push(`${pagePath}?q=${encodeURIComponent(searchQuery)}`);
  };
          ...
          <div className="flex flex-col sm:flex-row bg-white rounded-2xl shadow-lg p-3 sm:p-4 gap-3 sm:gap-4 border border-gray-200">
            <div className="relative flex-shrink-0 w-full sm:w-48">
              <select
                ...
                className="appearance-none w-full py-3 px-4 pr-10 rounded-xl border border-gray-300 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
              >
            ...
            <button
              ...
              className="bg-blue-500 text-white p-3 rounded-xl hover:bg-blue-400 transition flex items-center justify-center"
            >
