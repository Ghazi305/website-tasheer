      ...
      <div className="container mx-auto px-4 md:px-12 py-12">
        ...
        {filteredPosts.length === 0 ? (
          ...
          filteredPosts.map(post => (
            <div key={post.slug} className="flex flex-col md:flex-row gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-6">
              <div className="relative w-full md:w-64 h-48 md:h-40 flex-shrink-0">
                <Image src={post.imageSrc} alt={post.title} fill style={{ objectFit: 'cover' }} className="rounded-xl" />
              </div>
              <div className="flex-grow flex flex-col justify-between">
                ...
                <div className="mt-4">
                  <Link href={`/blog/${post.slug}`} className="text-blue-500 font-semibold hover:underline">
                    قراءة المزيد →
                  </Link>
                </div>
              </div>
